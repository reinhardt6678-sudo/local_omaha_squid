...
function decidePreflop(context: BotDecisionContext): BotDecision {
  ...
  if (faced === 0) {
    /* --- 无人加注：开池 / 跛入 / 弃牌 --- */
    const threshold = openThreshold(behind, active) * profile.openRange;
    ...
    const reraiseThreshold = reraiseBase * profile.threeBet;

    const baseCall = faced === 1 ? (inPosition ? 24 : 17) : inPosition ? 11 : 8;
    // 大盲已经投入了盲注，可以防守得更宽
    const isBigBlind = seat.bet >= state.config.bigBlind && faced === 1;
    const callThreshold = (baseCall + (isBigBlind ? 11 : 0)) * profile.callRange;
...
function decidePostflop(context: BotDecisionContext): BotDecision {
  ...
  const valueThreshold = Math.min(0.9, 0.55 + multiwayShift);
  const raiseThreshold = Math.min(0.93, 0.63 + multiwayShift);
  ...
  if (toCall > 0) {
    /* --- 面对下注 --- */
    // 需要的胜率就是底池赔率；跟注站会用比这更差的牌跟
    const requiredEquity = (potOdds * 1.04) / profile.callDown;
    ...
    const bluffRaiseProb = drawish ? 0.16 * profile.bluff * (behind === 0 ? 1.25 : 0.7) : 0;
    ...
    const bluffProb = bluffCandidate
      ? balancedBluffRatio * profile.bluff * (behind === 0 ? 1.15 : 0.75) * (opponents > 1 ? 0.5 : 1)
      : 0;
